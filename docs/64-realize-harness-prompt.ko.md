...
- `ClaudeCodeRealizer`는 루프를 위임한다는 점에서 예외이지만(61 문서의 '하네스
  속 하네스'), 프롬프트를 놓는 자리는 다른 Realizer와 같습니다. Claude Code에
  시스템 프롬프트를 통째로 **대체**하는 수단이 있어서, 이 문서의 고정부와
  가변부가 그대로 시스템 프롬프트 자리에 들어가고 킥오프만 유저 턴이 됩니다.
  대체하면 Claude Code 자신의 프롬프트뿐 아니라 그쪽 `CLAUDE.md` 주입도 함께
  사라지므로, 세션이 보는 지시문은 이 문서가 정한 것뿐입니다. 고정부와 가변부
  사이에는 경계 표식을 하나 끼워 두는데, 그 덕에 두 파트의 구분이 곧 프롬프트
  캐시의 경계가 됩니다.

  > 예전에는 이 자리에 "Claude Code의 자체 시스템 프롬프트를 대체할 수 없으므로
  > 세션 프롬프트(지시문)로 주입한다"고 적혀 있었습니다. 구현하면서 확인해 보니
  > 사실이 아니었고, 위 내용으로 정정했습니다.
