...
1. **해석(resolve)**: 절대 경로는 그대로 씁니다. 상대 경로는 `projectRoot`
   기준이 정본이고, 이 해석이 통하면 언제나 이쪽을 씁니다. 통하지 않을 때에
   한해 `outputDir` 기준 해석을 한 번 더 봅니다.

   기준이 둘인 이유는, 세션이 시간의 대부분을 `outputDir` 안에서 보내기
   때문입니다. 모델이 `implementations/foo.ts`라고 쓸 때 그것은 프로젝트
   최상단의 파일이 아니라 방금 자기가 만든 산출물을 가리킬 때가 훨씬 많고,
   기준을 하나만 인정하면 그때마다 거절 → 재시도로 턴 하나가 사라집니다.

   다만 보조 해석은 **없는 위치를 지어내서는 안 됩니다.** 조건 없이 재시도하게
   두면, 경계를 벗어나 거절되어야 할 경로가 `outputDir` 안의 그럴듯한 경로로
   바뀌어 돌아오고, 모델은 자기가 요청하지도 않은 파일을 조용히 받게 됩니다.
   에러보다 나쁜 결과입니다. 그래서 보조 해석에는 **그렇게 의도했다는 증거**를
   요구합니다.

   | 툴 계열 | 정본 해석이 통하지 않는다는 뜻 | 보조 해석에 요구하는 증거 |
   |---------|--------------------------------|---------------------------|
   | 읽기 | 그 경로에 파일이 없음 | 보조 경로에 파일이 실제로 있음 |
   | 쓰기 | 그 경로가 `outputDir` 밖 | 보조 경로의 **상위 디렉토리가 이미 존재**함 (파일 자체는 새로 만들어도 됨) |
   | 검증(`RunTests`) | 그 경로에 파일이 없음 | 보조 경로에 테스트 파일이 실제로 있음 |

   증거가 없으면 정본 해석을 그대로 들고 다음 단계로 갑니다. 에러 문구가 모델이
   실제로 넘긴 경로를 가리키게 하기 위해서입니다. 두 해석 모두 아래의 포함
   검사와 차단 목록을 똑같이 통과해야 하므로, 보조 해석이 정본보다 멀리 닿는
   일은 없습니다.
...
- `testFiles`는 `outputDir` 내부의 테스트 파일 경로들이어야 합니다(경계 검사
  적용). 빈 배열이면 세션 스코프의 전체 테스트 — 이 세션이 realize 중인 모든
  심볼(순환 그룹이면 전 멤버)의 autogen + ensure — 를 실행합니다. 스코프
  심볼의 autogen 테스트가 아직 없으면 red 결과와 함께 "유닛 테스트를 먼저
  작성하라"는 복구 힌트를 돌려줍니다.
...
**직렬화.** 병렬 realize(`-j`)에서는 여러 세션이 동시에 AskUser를 호출할 수
있습니다. 엔진은 질문 배치를 FIFO 큐로 직렬화하여 사람에게 **한 번에 한
배치씩** 전달합니다 — 먼저 물은 세션이 먼저 답을 받고, 한 배치의 실패가 뒤의
질문들을 막지 않습니다.
