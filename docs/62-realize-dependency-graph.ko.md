...
3의 확정 엣지가 **추정 그래프에 합류하여** 순서를 함께 구속합니다. 언급 기반
추정 엣지도 계속 유지됩니다 — ensure 하네스의 import가 언급 스캔으로 결정되므로,
언급 엣지를 버리면 하네스가 realize 순서보다 앞서 다른 심볼을 import하는 모순이
생길 수 있기 때문입니다. (스펙이 변경된 심볼의 옛 확정 엣지는 신뢰하지 않습니다.)

엣지 발견은 **파일 경계를 넘어서도 동일하게** 동작합니다. 다른 치즈 파일의
심볼 사용은 shim 경로 import — 예: `import { ShootingGame } from './example'`
([20 문서](20-module-resolution.ko.md) 참조) — 로 드러나므로, 위 1~3의
